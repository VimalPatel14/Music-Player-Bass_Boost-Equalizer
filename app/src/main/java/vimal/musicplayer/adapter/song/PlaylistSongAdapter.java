    ...
    public void onBindViewHolder(@NonNull final SongAdapter.ViewHolder holder, int position) {
        if (holder.getItemViewType() == OFFSET_ITEM) {
            ...
            if (holder.title != null) {
                holder.title.setText(Music_Player_Bass_MusicUtil.getPlaylistInfoString(activity, dataSet));
    ...
    public class ViewHolder extends AbsOffsetSongAdapter.ViewHolder {
        ...
        protected boolean onSongMenuItemClick(MenuItem item) {
            if (item.getItemId() == R.id.action_go_to_album) {
                ...
                Music_Player_Bass_NavigationUtil.goToAlbum(activity, dataSet.get(getAdapterPosition() - 1).albumId, albumPairs);
                return true;
            }

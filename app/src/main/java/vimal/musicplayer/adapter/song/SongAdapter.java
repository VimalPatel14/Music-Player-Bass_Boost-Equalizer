...
public class SongAdapter extends AbsMultiSelectAdapter<SongAdapter.ViewHolder, Song> implements MaterialCab.Callback, FastScrollRecyclerView.SectionedAdapter {
    ...
    protected void loadAlbumCover(Song song, final ViewHolder holder) {
        ...
        Music_Player_Bass_SongGlideRequest.Builder.from(Glide.with(activity), song)
    ...
    protected String getSongText(Song song) {
        return Music_Player_Bass_MusicUtil.getSongInfoString(song);
    }
    ...
    public String getSectionName(int position) {
        ...
        switch (Music_Player_Bass_PreferenceUtil.getInstance(activity).getSongSortOrder()) {
            case Music_Player_Bass_SortOrder.SongSortOrder.SONG_A_Z:
            case Music_Player_Bass_SortOrder.SongSortOrder.SONG_Z_A:
                sectionName = dataSet.get(position).title;
                break;
            case Music_Player_Bass_SortOrder.SongSortOrder.SONG_ALBUM:
                sectionName = dataSet.get(position).albumName;
                break;
            case Music_Player_Bass_SortOrder.SongSortOrder.SONG_ARTIST:
                sectionName = dataSet.get(position).artistName;
                break;
            case Music_Player_Bass_SortOrder.SongSortOrder.SONG_YEAR:
                return Music_Player_Bass_MusicUtil.getYearString(dataSet.get(position).year);
        }
            ...
            if (image != null && image.getVisibility() == View.VISIBLE) {
                switch (item.getItemId()) {
                    case R.id.action_go_to_album:
                        ...
                        Music_Player_Bass_NavigationUtil.goToAlbum(activity, getSong().albumId, albumPairs);
                        return true;
                }
            }
            return false;

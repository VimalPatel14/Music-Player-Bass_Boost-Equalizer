...
import vimal.musicplayer.BuildConfig;
import vimal.musicplayer.helper.Music_Player_Bass_StackBlur;
import vimal.musicplayer.util.Music_Player_Bass_ImageUtil;


public class Music_Player_Bass_BlurTransformation extends BitmapTransformation {
    ...
    private Music_Player_Bass_BlurTransformation(Builder builder) {
        super(builder.context);
        init(builder);
    }

    private Music_Player_Bass_BlurTransformation(Builder builder, BitmapPool bitmapPool) {
        super(bitmapPool);
        init(builder);
    }
    ...
    public static class Builder {
        ...
        public Music_Player_Bass_BlurTransformation build() {
            if (bitmapPool != null) {
                return new Music_Player_Bass_BlurTransformation(this, bitmapPool);
            }
            return new Music_Player_Bass_BlurTransformation(this);
        }
    }
    ...
    protected Bitmap transform(BitmapPool pool, Bitmap toTransform, int outWidth, int outHeight) {
        int sampling;
        if (this.sampling == 0) {
            sampling = Music_Player_Bass_ImageUtil.calculateInSampleSize(toTransform.getWidth(), toTransform.getHeight(), 100);
        } else {
        ...
        return Music_Player_Bass_StackBlur.blur(out, blurRadius);
    }

    @Override
    public String getId() {
        return "Music_Player_Bass_BlurTransformation(radius=" + blurRadius + ", sampling=" + sampling + ")";
    }
}

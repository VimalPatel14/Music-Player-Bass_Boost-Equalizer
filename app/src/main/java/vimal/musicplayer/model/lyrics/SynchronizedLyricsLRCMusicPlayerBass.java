package vimal.musicplayer.model.lyrics;

import java.util.regex.Matcher;
import java.util.regex.Pattern;

class SynchronizedLyricsLRCMusicPlayerBass extends Music_Player_Bass_AbsSynchronizedLyrics {
    ...
    @Override
    public SynchronizedLyricsLRCMusicPlayerBass parse(boolean check) {
        ...
        for (String line : lines) {
            ...
            Matcher attrMatcher = SynchronizedLyricsLRCMusicPlayerBass.LRC_ATTRIBUTE_PATTERN.matcher(line);
            if (attrMatcher.find()) {
                try {
                ...
                Matcher matcher = SynchronizedLyricsLRCMusicPlayerBass.LRC_LINE_PATTERN.matcher(line);
                if (matcher.find()) {
                    String time = matcher.group(1);
                    String text = matcher.group(2);

                    Matcher timeMatcher = SynchronizedLyricsLRCMusicPlayerBass.LRC_TIME_PATTERN.matcher(time);

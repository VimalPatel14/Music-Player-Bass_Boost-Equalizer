package vimal.musicplayer.service;
...
import vimal.musicplayer.appwidgets.AppWidgetBig;
import vimal.musicplayer.appwidgets.AppWidgetCard;
import vimal.musicplayer.appwidgets.AppWidgetClassic;
import vimal.musicplayer.appwidgets.AppWidgetSmall;
import vimal.musicplayer.glide.Music_Player_Bass_BlurTransformation;
import vimal.musicplayer.glide.Music_Player_Bass_SongGlideRequest;
import vimal.musicplayer.helper.Music_Player_Bass_ShuffleHelper;
import vimal.musicplayer.helper.StopWatch;
...
import vimal.musicplayer.provider.MusicPlaybackQueueStore;
import vimal.musicplayer.provider.Music_Player_Bass_SongPlayCountStore;
import vimal.musicplayer.service.notification.PlayingNotification;
import vimal.musicplayer.service.notification.PlayingNotificationImpl;
import vimal.musicplayer.service.notification.PlayingNotificationImpl24;
import vimal.musicplayer.service.playback.Playback;
...
public class MusicService extends Service implements SharedPreferences.OnSharedPreferenceChangeListener, Playback.PlaybackCallbacks {
    ...
    public static final String PHONOGRAPH_PACKAGE_NAME = "photovideomusicstudio.GalleryPhotoMusicPlayer";
    ...
    private static String getTrackUri(@NonNull Song song) {
        return Music_Player_Bass_MusicUtil.getSongFileUri(song.id).toString();
    }
    ...
    public void onCreate() {
        ...
        Music_Player_Bass_PreferenceUtil.getInstance(this).registerOnSharedPreferenceChangedListener(this);

        restoreState();

        mediaSession.setActive(true);

        sendBroadcast(new Intent("photovideomusicstudio.GalleryPhotoMusicPlayer.PHONOGRAPH_MUSIC_SERVICE_CREATED"));
    }
                    ...
                    case ACTION_PLAY_PLAYLIST:
                        ...
                        if (playlist != null) {
                            ...
                            if (playlist instanceof AbsCustomPlaylist) {
                                ...
                                playlistSongs = (ArrayList<Song>) (List) Music_Player_Bass_PlaylistSongLoader.getPlaylistSongList(getApplicationContext(), playlist.id);
                            }
    ...
    public void onDestroy() {
        ...
        Music_Player_Bass_PreferenceUtil.getInstance(this).unregisterOnSharedPreferenceChangedListener(this);
        wakeLock.release();

        sendBroadcast(new Intent("photovideomusicstudio.GalleryPhotoMusicPlayer.PHONOGRAPH_MUSIC_SERVICE_DESTROYED"));
    }
    ...
    public void initNotification() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N && !Music_Player_Bass_PreferenceUtil.getInstance(this).classicNotification()) {
    ...
    private void updateMediaSessionMetaData() {
        ...
        if (Music_Player_Bass_PreferenceUtil.getInstance(this).albumArtOnLockscreen()) {
            final Point screenSize = Music_Player_Bass_Util.getScreenSize(MusicService.this);
            final BitmapRequestBuilder<?, Bitmap> request = Music_Player_Bass_SongGlideRequest.Builder.from(Glide.with(MusicService.this), song)
                    .checkIgnoreMediaStore(MusicService.this)
                    .asBitmap().build();
            if (Music_Player_Bass_PreferenceUtil.getInstance(this).blurredAlbumArt()) {
                request.transform(new Music_Player_Bass_BlurTransformation.Builder(MusicService.this).build());
            }
    ...
    public void openQueue(@Nullable final ArrayList<Song> playingQueue, final int startPosition, final boolean startPlaying) {
        if (playingQueue != null && !playingQueue.isEmpty() && startPosition >= 0 && startPosition < playingQueue.size()) {
            ...
            if (shuffleMode == SHUFFLE_MODE_SHUFFLE) {
                Music_Player_Bass_ShuffleHelper.makeShuffleList(this.playingQueue, startPosition);
                position = 0;
            }
            if (startPlaying) {
                playSongAt(position);
    ...
    public void setShuffleMode(final int shuffleMode) {
        ...
        switch (shuffleMode) {
            case SHUFFLE_MODE_SHUFFLE:
                ...
                Music_Player_Bass_ShuffleHelper.makeShuffleList(this.getPlayingQueue(), getPosition());
                position = 0;
                break;
    ...
    private void handleChangeInternal(@NonNull final String what) {
        switch (what) {
            ...
            case META_CHANGED:
                ...
                Music_Player_Bass_HistoryStore.getInstance(this).addSongId(currentSong.id);
                if (songPlayCountHelper.shouldBumpPlayCount()) {
                    Music_Player_Bass_SongPlayCountStore.getInstance(this).bumpPlayCount(songPlayCountHelper.getSong().id);
                }
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        switch (key) {
            case Music_Player_Bass_PreferenceUtil.GAPLESS_PLAYBACK:
                ...
                break;
            case Music_Player_Bass_PreferenceUtil.ALBUM_ART_ON_LOCKSCREEN:
            case Music_Player_Bass_PreferenceUtil.BLURRED_ALBUM_ART:
                updateMediaSessionMetaData();
                break;
            case Music_Player_Bass_PreferenceUtil.COLORED_NOTIFICATION:
                updateNotification();
                break;
            case Music_Player_Bass_PreferenceUtil.CLASSIC_NOTIFICATION:
                initNotification();
                updateNotification();
                break;
        }
    ...
    private static final class PlaybackHandler extends Handler {
        ...
        public void handleMessage(@NonNull final Message msg) {
            ...
            switch (msg.what) {
                case DUCK:
                    if (Music_Player_Bass_PreferenceUtil.getInstance(service).audioDucking()) {

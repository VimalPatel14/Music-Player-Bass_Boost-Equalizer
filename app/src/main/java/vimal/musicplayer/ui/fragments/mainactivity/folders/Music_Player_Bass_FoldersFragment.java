package vimal.musicplayer.ui.fragments.mainactivity.folders;
...
import vimal.musicplayer.adapter.SongFileAdapter;
...
import vimal.musicplayer.misc.DialogAsyncTask;
import vimal.musicplayer.misc.UpdateToastMediaScannerCompletionListener;
...
import vimal.musicplayer.views.Music_Player_Bass_BreadCrumbLayout;
...
public class Music_Player_Bass_FoldersFragment extends AbsMainActivityFragment implements MainActivityMusicPlayerBass.MainActivityFragmentCallbacks, CabHolder, Music_Player_Bass_BreadCrumbLayout.SelectionCallback, SongFileAdapter.Callbacks, AppBarLayout.OnOffsetChangedListener, LoaderManager.LoaderCallbacks<List<File>> {
    ...
    Music_Player_Bass_BreadCrumbLayout breadCrumbs;
    ...
    public Music_Player_Bass_FoldersFragment() {
    }

    public static Music_Player_Bass_FoldersFragment newInstance(Context context) {
        return newInstance(Music_Player_Bass_PreferenceUtil.getInstance(context).getStartDirectory());
    }

    public static Music_Player_Bass_FoldersFragment newInstance(File directory) {
        Music_Player_Bass_FoldersFragment frag = new Music_Player_Bass_FoldersFragment();
        Bundle b = new Bundle();
    ...
    public void setCrumb(Music_Player_Bass_BreadCrumbLayout.Crumb crumb, boolean addToHistory) {
    ...
    private void saveScrollPosition() {
        Music_Player_Bass_BreadCrumbLayout.Crumb crumb = getActiveCrumb();
        if (crumb != null) {
    ...
    @Nullable
    private Music_Player_Bass_BreadCrumbLayout.Crumb getActiveCrumb() {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            setCrumb(new Music_Player_Bass_BreadCrumbLayout.Crumb(Music_Player_Bass_FileUtil.safeGetCanonicalFile((File) getArguments().getSerializable(PATH))), true);
        } else {
    ...
    private void setUpRecyclerView() {
        Music_Player_Bass_ViewUtil.setUpFastScrollRecyclerViewColor(getActivity(), recyclerView, ThemeStore.accentColor(getActivity()));
    ...
    public static final FileFilter AUDIO_FILE_FILTER = file -> !file.isHidden() && (file.isDirectory() ||
            Music_Player_Bass_FileUtil.fileIsMimeType(file, "audio/*", MimeTypeMap.getSingleton()) ||
            Music_Player_Bass_FileUtil.fileIsMimeType(file, "application/ogg", MimeTypeMap.getSingleton()));

    @Override
    public void onCrumbSelection(Music_Player_Bass_BreadCrumbLayout.Crumb crumb, int index) {
        setCrumb(crumb, true);
    }
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case R.id.action_go_to_start_directory:
                setCrumb(new Music_Player_Bass_BreadCrumbLayout.Crumb(Music_Player_Bass_FileUtil.safeGetCanonicalFile(Music_Player_Bass_PreferenceUtil.getInstance(getActivity()).getStartDirectory())), true);
                return true;
    ...
    public void onFileSelected(File file) {
        final File canonicalFile = Music_Player_Bass_FileUtil.safeGetCanonicalFile(file); // important as we compare the path value later
        if (canonicalFile.isDirectory()) {
            setCrumb(new Music_Player_Bass_BreadCrumbLayout.Crumb(canonicalFile), true);
        } else {
                ...
                Snackbar.make(coordinatorLayout, R.string.some_files_are_not_listed_in_the_media_store, Snackbar.LENGTH_LONG)
                        .setAction(R.string.action_scan, v -> {
                            ...
                            for (int i = 0; i < files.size(); i++) {
                                paths[i] = Music_Player_Bass_FileUtil.safeGetCanonicalPath(files.get(i));
                            }
                            scanPaths(paths);
                        })
                        .setActionTextColor(ThemeStore.accentColor(getActivity()))
            ...
            popupMenu.setOnMenuItemClickListener(item -> {
                ...
                switch (itemId) {
                    ...
                    case R.id.action_set_as_start_directory:
                        Music_Player_Bass_PreferenceUtil.getInstance(getActivity()).setStartDirectory(file);
            ...
            popupMenu.setOnMenuItemClickListener(item -> {
                ...
                switch (itemId) {
                    ...
                    case R.id.action_delete_from_device:
                        new ListSongsAsyncTask(getActivity(), null, (songs, extra) -> {
                            if (!songs.isEmpty()) {
                                ...
                                Snackbar.make(coordinatorLayout, Html.fromHtml(String.format(getString(R.string.not_listed_in_media_store), file.getName())), Snackbar.LENGTH_LONG)
                                        .setAction(R.string.action_scan, v -> scanPaths(new String[]{Music_Player_Bass_FileUtil.safeGetCanonicalPath(file)}))
                                        .setActionTextColor(ThemeStore.accentColor(getActivity()))
                                        .show();
                            }
                        }).execute(new ListSongsAsyncTask.LoadingInfo(toList(file), AUDIO_FILE_FILTER, getFileComparator()));
                        return true;
                    case R.id.action_scan:
                        scanPaths(new String[]{Music_Player_Bass_FileUtil.safeGetCanonicalPath(file)});
                        return true;
                }
                return false;
            });
    ...
    private static class AsyncFileLoader extends WrappedAsyncTaskLoader<List<File>> {
        private WeakReference<Music_Player_Bass_FoldersFragment> fragmentWeakReference;

        public AsyncFileLoader(Music_Player_Bass_FoldersFragment musicPlayerBassFoldersFragment) {
            super(musicPlayerBassFoldersFragment.getActivity());
            fragmentWeakReference = new WeakReference<>(musicPlayerBassFoldersFragment);
        }

        @Override
        public List<File> loadInBackground() {
            Music_Player_Bass_FoldersFragment musicPlayerBassFoldersFragment = fragmentWeakReference.get();
            File directory = null;
            if (musicPlayerBassFoldersFragment != null) {
                Music_Player_Bass_BreadCrumbLayout.Crumb crumb = musicPlayerBassFoldersFragment.getActiveCrumb();
                if (crumb != null) {
                    directory = crumb.getFile();
                }
            }
            if (directory != null) {
                List<File> files = Music_Player_Bass_FileUtil.listFiles(directory, AUDIO_FILE_FILTER);
                Collections.sort(files, musicPlayerBassFoldersFragment.getFileComparator());
                return files;
            } else {
    ...
    private static class ListSongsAsyncTask extends ListingFilesDialogAsyncTask<ListSongsAsyncTask.LoadingInfo, Void, ArrayList<Song>> {
        ...
        protected ArrayList<Song> doInBackground(LoadingInfo... params) {
            try {
                ...
                List<File> files = Music_Player_Bass_FileUtil.listFilesDeep(info.files, info.fileFilter);
                ...
                return Music_Player_Bass_FileUtil.matchFilesWithMediaStore(context, files);
            } catch (Exception e) {
                e.printStackTrace();
                cancel(false);
                return null;
        ...
        protected String[] doInBackground(LoadingInfo... params) {
            try {
                ...
                if (info.file.isDirectory()) {
                    List<File> files = Music_Player_Bass_FileUtil.listFilesDeep(info.file, info.fileFilter);
                    ...
                    for (int i = 0; i < files.size(); i++) {
                        File f = files.get(i);
                        paths[i] = Music_Player_Bass_FileUtil.safeGetCanonicalPath(f);

                        if (isCancelled() || checkCallbackReference() == null) return null;
                    }
                } else {
                    paths = new String[1];
                    paths[0] = Music_Player_Bass_FileUtil.safeGetCanonicalPath(info.file);
                }

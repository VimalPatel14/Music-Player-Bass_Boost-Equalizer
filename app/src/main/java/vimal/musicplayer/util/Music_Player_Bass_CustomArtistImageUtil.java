...
public class Music_Player_Bass_CustomArtistImageUtil {
    ...
    private static Music_Player_Bass_CustomArtistImageUtil sInstance;

    private final SharedPreferences mPreferences;

    private Music_Player_Bass_CustomArtistImageUtil(@NonNull final Context context) {
        mPreferences = context.getApplicationContext().getSharedPreferences(CUSTOM_ARTIST_IMAGE_PREFS, Context.MODE_PRIVATE);
    }

    public static Music_Player_Bass_CustomArtistImageUtil getInstance(@NonNull final Context context) {
        if (sInstance == null) {
            sInstance = new Music_Player_Bass_CustomArtistImageUtil(context.getApplicationContext());
        }
        return sInstance;
    }
                        ...
                        new AsyncTask<Void, Void, Void>() {
                            ...
                            protected Void doInBackground(Void... params) {
                                ...
                                try {
                                    ...
                                    succesful = Music_Player_Bass_ImageUtil.resizeBitmap(resource, 2048).compress(Bitmap.CompressFormat.JPEG, 100, os);
                                    os.close();
                                } catch (IOException e) {
                                    Toast.makeText(App.getInstance(), e.toString(), Toast.LENGTH_LONG).show();
                                }
                                ...
                                if (succesful) {
                                    ...
                                    Music_Player_Bass_ArtistSignatureUtil.getInstance(App.getInstance()).updateArtistSignature(artist.getName());

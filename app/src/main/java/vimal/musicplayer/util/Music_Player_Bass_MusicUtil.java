...
public class Music_Player_Bass_MusicUtil {
    ...
    public static String getArtistInfoString(@NonNull final Context context, @NonNull final Artist artist) {
        ...
        return Music_Player_Bass_MusicUtil.buildInfoString(
            Music_Player_Bass_MusicUtil.getAlbumCountString(context, albumCount),
            Music_Player_Bass_MusicUtil.getSongCountString(context, songCount)
        );
    }
    ...
    public static String getGenreInfoString(@NonNull final Context context, @NonNull final Genre genre) {
        ...
        return Music_Player_Bass_MusicUtil.getSongCountString(context, songCount);
    }
    ...
    public static String getPlaylistInfoString(@NonNull final Context context, @NonNull List<Song> songs) {
        ...
        return Music_Player_Bass_MusicUtil.buildInfoString(
            Music_Player_Bass_MusicUtil.getSongCountString(context, songs.size()),
        ...
        try {
            ...
            if (cursor != null) {
                ...
                while (!cursor.isAfterLast()) {
                    ...
                    final Song song = Music_Player_Bass_SongLoader.getSong(context, id);
    ...
    public static Playlist getFavoritesPlaylist(@NonNull final Context context) {
        return Music_Player_Bass_PlaylistLoader.getPlaylist(context, context.getString(R.string.favorites));
    }

    private static Playlist getOrCreateFavoritesPlaylist(@NonNull final Context context) {
        return Music_Player_Bass_PlaylistLoader.getPlaylist(context, Music_Player_Bass_PlaylistsUtil.createPlaylist(context, context.getString(R.string.favorites)));
    }

    public static boolean isFavorite(@NonNull final Context context, @NonNull final Song song) {
        return Music_Player_Bass_PlaylistsUtil.doPlaylistContains(context, getFavoritesPlaylist(context).id, song.id);
    }

    public static void toggleFavorite(@NonNull final Context context, @NonNull final Song song) {
        if (isFavorite(context, song)) {
            Music_Player_Bass_PlaylistsUtil.removeFromPlaylist(context, song, getFavoritesPlaylist(context).id);
        } else {
            Music_Player_Bass_PlaylistsUtil.addToPlaylist(context, song, getOrCreateFavoritesPlaylist(context).id, false);
        }
    }
    ...
    public static String getLyrics(Song song) {
        ...
        if (lyrics == null || lyrics.trim().isEmpty() || !Music_Player_Bass_AbsSynchronizedLyrics.isSynchronized(lyrics)) {
            File dir = file.getAbsoluteFile().getParentFile();

            if (dir != null && dir.exists() && dir.isDirectory()) {
                String format = ".*%s.*\\.(lrc|txt)";
                String filename = Pattern.quote(Music_Player_Bass_FileUtil.stripExtension(file.getName()));
                ...
                if (files != null && files.length > 0) {
                    for (File f : files) {
                        try {
                            String newLyrics = Music_Player_Bass_FileUtil.read(f);
                            if (newLyrics != null && !newLyrics.trim().isEmpty()) {
                                if (Music_Player_Bass_AbsSynchronizedLyrics.isSynchronized(newLyrics)) {
                                    return newLyrics;
                                }
                                lyrics = newLyrics;
                            }
